...
namespace PoolTracker.Repository.Repository
{
    public class LeaderBoardRepository : ILeaderBoardRepository
    {
        ...
        {
            var query = ";WITH cte_player_wins (player_id, wins) AS ( SELECT p.id AS PlayerId, count(1) AS Wins FROM Matches as m " +
                        ...
                        "GROUP BY p.id " +
                        ") " +
                        "SELECT p.*, isnull(w.wins, 0) as wins, isnull(l.losses, 0) as losses, ROW_NUMBER() OVER(ORDER BY(coalesce(w.wins, 0) + coalesce(l.losses, 0) + coalesce(l.losses, 0) + coalesce(l.losses, 0) + coalesce(b.balls, 0)) asc, wins desc) as position " +
                        "FROM players as p " +
